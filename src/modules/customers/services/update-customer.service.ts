...
export class UpdateCustomerService implements UpdateCustomerServiceInterface {
  async update(
    ...
    const newIdCustomerExistsOnCustomerKeys =
      allCustomersKey.includes(keyWithNewIdCustomer);

    const idCustomerToUpdateIsDifferentOfNewIdCustomer =
      idCustomerToUpdate !== newIdToCustomer;
    const newIdCustomerAlreadyExistsOnCustomerKeys =
      idCustomerToUpdateIsDifferentOfNewIdCustomer &&
      newIdCustomerExistsOnCustomerKeys;
    ...
    const customerFinded = await redisClient.get(
    ...
    if (idCustomerToUpdateIsDifferentOfNewIdCustomer) {
      ...
      await redisClient.del(oldKeyWithCustomerUpdatedId);
    }
